...
function lookup(args, emitter, currentDomain, principalDomain, cb, delegationChain) {
  if (!currentDomain) currentDomain = principalDomain;
  if (!principalDomain) principalDomain = currentDomain;
  ...
  fetchWellKnown(args, emitter, currentDomain, principalDomain, function(err, unparsedDoc) {
    ...
    else if (supportDoc.type === 'delegation')
    {
      ...
      // recurse
      return lookup(args, emitter, currentDomain, principalDomain, cb, delegationChain);
    }
...
module.exports = function (args, browserid, currentDomain, principalDomain, cb) {
  lookup(args, browserid, currentDomain, principalDomain, function(err, details) {
    // if there is an error, then let's try the fallback if configured
    if (err && args.fallback) {
      return lookup(args, browserid, args.fallback, principalDomain, cb, [ args.fallback ]);
    }
    cb(err, details);
  });
};
