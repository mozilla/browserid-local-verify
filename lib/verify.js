...
util = require('util'),
dbug = require('dbug')('browserid-local-verify:verify'),
...
function verify(browserid, args, cb) {
  ...
  try {
    ...
    principalDomain = validation.validateEmail(email).host;
  } catch(e) {
  ...
  jwcrypto.cert.verifyBundle(
    ...
    function(issuer, next) {
      ...
      if (args.trustedIssuers && args.trustedIssuers.indexOf(ultimateIssuer) !== -1) {
        dbug('trusted issuer "%s" found: %j', ultimateIssuer, obj);
        cb(null, obj);
      }
      ...
      // expected issuer (by the BrowserID protocol) for that email domain.
        ...
        dbug('verifying issuer "%s" can speak for principal "%s"', ultimateIssuer, principalDomain);
